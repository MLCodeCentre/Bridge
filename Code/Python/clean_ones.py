...
def loadOnes(thresh, seconds):
    ...
    file_dir = os.path.join('Data', 'Clean Twos', thresh, '{}secs'.format(str(seconds)),'*')
...
def main(args):
    ...
    chans, labels = createChansAndLabels(args.thresh, args.chan, args.window_size, args.seconds, set="B")
    
    data_out_dir = os.path.join('Data', 'Clean Twos', args.thresh, '{}secs'.format(str(args.seconds)))
